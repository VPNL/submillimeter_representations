...
outpath = sprintf('%s/glm_metrics_6depth.mat', output_dir);
					...
					for partition_idx = 1:length(partitions)
						...
						for contrast_idx = 1:length(contrasts)
							...
							for depth = 1:6
								...
								metric_values = add_noise(...
									glm_metric, ...
									noise_type, ...
									noise_magnitude ...
								);
								data_struct.(FSID_session_str).(glm_dir).(split).(metric).(noise_type).(partition).(contrast)(:, depth) = metric_values;
							end
						end % contrast
					end % partition
				end % noise_type
